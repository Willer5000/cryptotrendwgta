...
def find_support_resistance(df, window):
    try:
        ...
        df['high_roll'] = df['high'].rolling(window=window, center=True).max()
        df['low_roll'] = df['low'].rolling(window=window, center=True).min()
...
def detect_divergence(df, lookback):
    try:
        ...
        if max_idx is not None:
            ...
            if current_price > price_high and current_rsi < rsi_high and current_rsi > 65:
                return 'bearish'
        ...
        if min_idx is not None:
            ...
            if current_price < price_low and current_rsi > rsi_low and current_rsi < 35:
                return 'bullish'
...
# Calcular distancia al nivel más cercano (corregido)
...
# Analizar una criptomoneda (criterios optimizados)
def analyze_crypto(symbol, params):
    ...
    try:
        ...
        # Calcular probabilidades (umbral reducido a 55%)
        long_prob = 0
        short_prob = 0
        ...
        if volume_class in ['Medio', 'Alto', 'Muy Alto']: long_prob += 15
        ...
        if volume_class in ['Medio', 'Alto', 'Muy Alto']: short_prob += 15
        ...
        # Señales LONG (umbral reducido a 55%)
        long_signal = None
        if long_prob >= 55 and volume_class in ['Medio', 'Alto', 'Muy Alto']:
            ...
            entry = min(next_resistances) * 1.01 if next_resistances else last['close'] * 1.01
            
            # Encontrar el soporte más cercano por debajo para SL
            next_supports = [s for s in supports if s < entry]
            sl = max(next_supports) * 0.99 if next_supports else entry * (1 - params['max_risk_percent']/100)
        ...
        # Señales SHORT (umbral reducido a 55%)
        short_signal = None
        if short_prob >= 55 and volume_class in ['Medio', 'Alto', 'Muy Alto']:
            ...
            entry = max(next_supports) * 0.99 if next_supports else last['close'] * 0.99
            
            # Encontrar la resistencia más cercana por encima para SL
            next_resistances = [r for r in resistances if r > entry]
            sl = min(next_resistances) * 1.01 if next_resistances else entry * (1 + params['max_risk_percent']/100)
...
def background_update():
    while True:
        try:
            ...
            with data_lock:
                global_state['long_signals'] = long_signals
                global_state['short_signals'] = short_signals
                global_state['scatter_data'] = scatter_data
                global_state['last_update'] = datetime.now()
                global_state['cryptos_analyzed'] = analyzed_count
            
            logger.info(f"Actualización completada: {len(long_signals)} LONG, {len(short_signals)} SHORT, {analyzed_count} cryptos")
        except Exception as e:
...
def index():
    with data_lock:
        long_signals = global_state['long_signals']
        short_signals = global_state['short_signals']
        scatter_data = global_state['scatter_data']
        last_update = global_state['last_update']
        cryptos_analyzed = global_state['cryptos_analyzed']
...
def get_chart(symbol, signal_type):
    ...
    with data_lock:
        signals = global_state['long_signals'] if signal_type == 'long' else global_state['short_signals']
